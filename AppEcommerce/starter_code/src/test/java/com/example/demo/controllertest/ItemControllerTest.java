...
class ItemControllerTest {
	 ...
	 */
	@Test
	@DisplayName("Test method to test whether correct status is returned on fetching complete list of items")
	 ...
	 */
	@Test
	@DisplayName("Test method to test item fetch on the basis of item name")
	public void getItemByName() throws Exception {
		...
		mvcResult = mockMvc.perform(
				MockMvcRequestBuilders.get("/api/item/name/{name}", "Plates").accept(MediaType.APPLICATION_JSON))
	 ...
	 */
	@Test
	@DisplayName("Test method to test item fetch on the basis of item id")
	public void getItemById() throws Exception {
		...
		mvcResult = mockMvc.perform(MockMvcRequestBuilders.get("/api/item/{id}", 10).accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isNotFound()).andReturn();
	}
}
