...
class OrderControllerTest {
	...
	public void initUserCreation() throws Exception {
		...
		userRequest.setUsername("jwttestname4");
		...
		cartRequest.setUsername("jwttestname4");
	...
	public void testOrderCreationAndHistory() throws Exception {
		...
		MvcResult result = mockMvc.perform(MockMvcRequestBuilders.post("/api/order/submit/{username}", "jwttestname4")
				.accept(MediaType.APPLICATION_JSON).header("Authorization", request.getParameter("Authorization")))
				.andExpect(status().isOk()).andReturn();
		...
		mockMvc.perform(MockMvcRequestBuilders.post("/api/order/submit/{username}", "jwttestname4")
		...
		MvcResult mvcResult = mockMvc.perform(MockMvcRequestBuilders.get("/api/order/history/{username}", "jwttestname4")
				.accept(MediaType.APPLICATION_JSON).header("Authorization", request.getParameter("Authorization")))
				.andExpect(status().isOk()).andReturn();
		...
		mockMvc.perform(MockMvcRequestBuilders.get("/api/order/history/{username}", "jwttestname4")
				.accept(MediaType.APPLICATION_JSON)).andExpect(status().isUnauthorized());
	}
}
